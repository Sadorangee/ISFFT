module ISFFT_3D_FFT
   ...
   use ISFFT_Buffer
   use ISFFT_LocalTranspose
   use ISFFT_GlobalTranspose
   use ISFFT_FFT_kernel
   implicit none

contains

   subroutine FFT_x_block(phi_re_inout,phi_im_inout)
      real(kind=ISFFT_REAL_KIND), dimension(1:nx, 1:ny, 1:nz), intent(inout) :: phi_re_inout, phi_im_inout
      ...
      do kk = 1, nz
         do jj = 1, ny
            ...
            call MPI_allREDUCE(x_sendbuf, x_recvbuf, nx_global, ISFFT_DATA_TYPE, MPI_SUM, &
               MPI_COMM_X, ierr)
            call MPI_allREDUCE(x_im_sendbuf, x_im_recvbuf, nx_global, ISFFT_DATA_TYPE, MPI_SUM, &
               MPI_COMM_X, ierr)

            call ISFFT_1D_FFT_r2c(nx_global, x_recvbuf, x_im_recvbuf, 1)
      ...
      if (i_offset(npx) .le. halfspectrum_x) then
         do kk = 1, nz
            do ii = 1, min(nx,(halfspectrum_x - i_offset(npx) + 1))
               ...
               call MPI_allREDUCE(y_sendbuf, y_recvbuf, ny_global, ISFFT_DATA_TYPE, MPI_SUM, &
                  MPI_COMM_Y, ierr)
               call MPI_allREDUCE(y_im_sendbuf, y_im_recvbuf, ny_global, ISFFT_DATA_TYPE, MPI_SUM, &
                  MPI_COMM_Y, ierr)

               call ISFFT_1D_FFT_c2c(ny_global, y_recvbuf, y_im_recvbuf, 1)
      ...
      if (i_offset(npx) .le. halfspectrum_x) then
         do jj = 1, ny
            do ii = 1, min(nx,(halfspectrum_x - i_offset(npx) + 1))
               ...
               call MPI_allREDUCE(z_sendbuf, z_recvbuf, nz_global, ISFFT_DATA_TYPE, MPI_SUM, &
                  MPI_COMM_Z, ierr)
               call MPI_allREDUCE(z_im_sendbuf, z_im_recvbuf, nz_global, ISFFT_DATA_TYPE, MPI_SUM, &
                  MPI_COMM_Z, ierr)

               call ISFFT_1D_FFT_c2c(nz_global, z_recvbuf, z_im_recvbuf, 1)
   ...
   subroutine FFT_x_slab(phi_re_inout,phi_im_inout)
      real(kind=ISFFT_REAL_KIND), dimension(1:nx, 1:ny, 1:nz), intent(inout) :: phi_re_inout, phi_im_inout
      ! real(kind=ISFFT_REAL_KIND), dimension(:,:,:), intent(inout) :: phi_re_out, phi_im_out

      integer :: i, j, k, ierr
      !-------------------------xFFT + 0*lt + 0*gt------------------------------!
      do k = 1, nz
         do j = 1, ny
            call ISFFT_1D_FFT_r2c(nx_global, phi_re_inout(:,j,k), phi_im_inout(:,j,k), 1)
         enddo
      enddo
   ...
   subroutine FFT_y_slab(phi_re_in,phi_im_in,phi_re_out,phi_im_out)
      real(kind=ISFFT_REAL_KIND), dimension(1:nx, 1:ny, 1:nz), intent(inout) :: phi_re_in, phi_im_in
      real(kind=ISFFT_REAL_KIND), dimension(:,:,:), intent(inout) :: phi_re_out, phi_im_out
      ...
      if (i_offset(npx) .le. halfspectrum_x) then
         do k = 1, nz
            do j = 1, min(nx,(halfspectrum_x - i_offset(npx) + 1))
               call ISFFT_1D_FFT_c2c(ny_global, phi_re_out(:,j,k), phi_im_out(:,j,k), 1)
            enddo
         enddo
      endif
   ...
   subroutine FFT_z_slab(phi_re_in,phi_im_in,phi_re_out,phi_im_out)
      real(kind=ISFFT_REAL_KIND), dimension(1:ny, 1:nx, 1:nz) &
      ...
      if (i_offset(npx) .le. halfspectrum_x) then
         do k = 1, ny_global/npz0
            do j = 1, min(nx_global/npx0,(halfspectrum_x - i_offset(npx) + 1))
               call ISFFT_1D_FFT_c2c(nz_global, phi_re_out(:,j,k), phi_im_out(:,j,k), 1)
            enddo
         enddo
      endif
   ...
   subroutine iFFT_z_slab(phi_re_inout,phi_im_inout)
      real(kind=ISFFT_REAL_KIND), dimension(1:nz_global/npy0, 1:nx_global/npx0, 1:ny_global/npz0) &
      ...
      if (i_offset(npx) .le. halfspectrum_x) then
         do k = 1, ny_global/npz0
            do j = 1, min(nx_global/npx0,(halfspectrum_x - i_offset(npx) + 1))
               call ISFFT_1D_FFT_c2c(nz_global, phi_re_inout(:,j,k), phi_im_inout(:,j,k), -1)
            enddo
         enddo
      endif
   ...
   subroutine iFFT_x_slab(phi_re_in,phi_im_in,phi_re_out,phi_im_out)
      real(kind=ISFFT_REAL_KIND), dimension(1:ny_global/npy0, 1:nx_global/npx0, 1:nz_global/npz0) &
         , intent(inout) :: phi_re_in, phi_im_in
      real(kind=ISFFT_REAL_KIND), dimension(:,:,:), intent(inout) :: phi_re_out, phi_im_out
      ...
      do k = 1, nz_global/npz0
         do j = 1, ny_global/npy0
            call ISFFT_1D_FFT_r2c(nx_global, phi_re_out(:,j,k), phi_im_out(:,j,k), -1)
         enddo
      enddo
   ...
   subroutine FFT_y_pencil(phi_re_in,phi_im_in,phi_re_out,phi_im_out)
      real(kind=ISFFT_REAL_KIND), dimension(1:nx, 1:ny, 1:nz) &
         , intent(inout) :: phi_re_in, phi_im_in
      real(kind=ISFFT_REAL_KIND), dimension(:,:,:), intent(inout) :: phi_re_out, phi_im_out
      integer :: i, j, k, ierr
      !-------------------------yFFT + 1*lt + 1*gt------------------------------!
   ...
   subroutine FFT_z_pencil(phi_re_in,phi_im_in,phi_re_out,phi_im_out)
      real(kind=ISFFT_REAL_KIND), dimension(1:ny_global/npx0, 1:nx_global/npy0, 1:nz_global/npz0) &
         , intent(inout) :: phi_re_in,phi_im_in
      real(kind=ISFFT_REAL_KIND), dimension(:,:,:), intent(inout) :: phi_re_out, phi_im_out
      integer :: i, j, k, ierr
      !-------------------------zFFT + 1*lt + 1*gt------------------------------!
   ...
   subroutine iFFT_z_pencil(phi_re_inout,phi_im_inout)
      real(kind=ISFFT_REAL_KIND), dimension(1:nz_global/npx0, 1:nx_global/npy0, 1:ny_global/npz0) &
         , intent(inout) :: phi_re_inout, phi_im_inout
      ! real(kind=ISFFT_REAL_KIND), dimension(:,:,:), intent(inout) :: phi_re_out, phi_im_out
      integer :: i, j, k, ierr
      !-------------------------zIFFT + 0*lt + 0*gt------------------------------!
