module SPLASH_Poisson_Algebra
 ...
 subroutine Algebra_Periodic_Poisson_r2c(phi_re_inout, phi_im_inout)
    ...
    real(kind=SPLASH_REAL_KIND), intent(inout) :: phi_re_inout(:,:,:), phi_im_inout(:,:,:)
    real(kind=SPLASH_REAL_KIND) :: sxx, syy, szz
    real(kind=SPLASH_REAL_KIND) :: kxi_scaled, keta_scaled, kzeta_scaled, k_squared_total, inv_k_squared
    real(kind=SPLASH_REAL_KIND), parameter :: PI = 3.1415926535897932384626433832795_SPLASH_REAL_KIND

    ! Pre-compute constants
    sxx = 2.0_SPLASH_REAL_KIND * PI / SLx
    syy = 2.0_SPLASH_REAL_KIND * PI / SLy
    szz = 2.0_SPLASH_REAL_KIND * PI / SLz
    ...
    if (If_3dfft_decomp == 0) then
       ...
       do k3 = 1, kz
          ...
          if (If_scheme == 1) kzeta_scaled = 2.0_SPLASH_REAL_KIND * sin(PI * k_zeta / nz0) / (SLz / nz0)

          do k2 = 1, ky
             k_eta = k2 + npy * ny - 1
             if (k_eta >= ny0/2) k_eta = k_eta - ny0
             if (If_scheme == 0) keta_scaled = k_eta * syy
             if (If_scheme == 1) keta_scaled = 2.0_SPLASH_REAL_KIND * sin(PI * k_eta / ny0) / (SLy / ny0)

             do k1 = 1, kx
                k_xi = k1 + npx * nx - 1
                if (k_xi >= nx0/2) cycle
                if (If_scheme == 0) kxi_scaled = k_xi * sxx
                if (If_scheme == 1) kxi_scaled = 2.0_SPLASH_REAL_KIND * sin(PI * k_xi / nx0) / (SLx / nx0)

                k_squared_total = kxi_scaled**2 + keta_scaled**2 + kzeta_scaled**2

                if (k_squared_total > 0.0_SPLASH_REAL_KIND) then
                   inv_k_squared = -1.0_SPLASH_REAL_KIND / k_squared_total
                   phi_re_inout(k1, k2, k3) = phi_re_inout(k1, k2, k3) * inv_k_squared
                   phi_im_inout(k1, k2, k3) = phi_im_inout(k1, k2, k3) * inv_k_squared
                else
                   phi_re_inout(k1, k2, k3) = 0.0_SPLASH_REAL_KIND
                   phi_im_inout(k1, k2, k3) = 0.0_SPLASH_REAL_KIND
                endif
             enddo
          enddo
       enddo
...
end module SPLASH_Poisson_Algebra